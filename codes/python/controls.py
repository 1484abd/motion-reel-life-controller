...
from axis import world_to_player
        ...
        while self.running:
            ...
            if self.enabled and snap["tracking_enabled"]:
                ...
                if self._last_pos is not None:
                    dx_world = x - self._last_pos[0]
                    dy_world = y - self._last_pos[1]

                    local_right, local_forward = world_to_player(dx_world, dy_world, angle)

                    per_cm = self.settings.get("movement_sensitivity") or 0.1

                    if local_forward > 1:
                        self._press_key("w", min(abs(local_forward) * per_cm, 0.3))
                    elif local_forward < -1:
                        self._press_key("s", min(abs(local_forward) * per_cm, 0.3))

                    if local_right > 1:
                        self._press_key("d", min(abs(local_right) * per_cm, 0.3))
                    elif local_right < -1:
                        self._press_key("a", min(abs(local_right) * per_cm, 0.3))
                self._last_pos = (x, y)
